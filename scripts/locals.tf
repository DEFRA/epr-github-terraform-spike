locals {
  ...
  #    Explicitly excludes terraform_service_account. Whether a given
  #    membership already exists on GitHub is no longer a YAML flag —
  #    see imports.tf, which checks live via scripts/check_existence.py.
  ...
  # 4. Flatten repository-team permissions. Whether a given attachment
  #    already exists on GitHub is no longer a YAML flag — see imports.tf.
  ...
  settings_defaults = {
    branch_protection = {
      ...
      require_pull_request              = true  # set false for a repo that shouldn't get PR-review enforcement at all
